      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="sm:hidden space-y-3">
            {isLoading ? (
              Array.from({ length: 5 }).map((_, index) => (
                <div key={`loading-mobile-${index}`} className="rounded-lg border p-4 shadow-sm bg-white dark:bg-gray-900">
                  <div className="space-y-2">
              ...
              <div className="rounded-lg border p-4 shadow-sm bg-white dark:bg-gray-900 text-red-500 text-center">
                Error loading product data. Please try again.
              </div>
            ) : !productBatchesData || productBatchesData.data.length === 0 ? (
              <div className="rounded-lg border p-4 shadow-sm bg-white dark:bg-gray-900 text-center">
                No products found. Try adjusting your search.
              </div>
            ) : (
              productBatchesData.data.map((product) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge className="text-xs">{product.totalQuantity > 0 ? 'In Stock' : 'Out of Stock'}</Badge>
                  </div>
                  <div className="text-xs text-muted-foreground mb-1">SKU: {product.sku}</div>
                  <div className="text-xs text-muted-foreground mb-2">Category: {product.categories?.length ? product.categories[0] : 'N/A'}</div>
                  <div className="flex flex-wrap gap-2 text-sm mb-2">
                    <span>HSN: <span className="font-mono">{product.hsnCode}</span></span>
                    <span>Qty: <span className="font-semibold">{product.totalProductCount || 0}</span></span>
                  </div>
                  <div className="flex flex-wrap gap-2 text-xs text-muted-foreground mb-2">
                    <span>Status: <Badge variant={product.totalQuantity > 0 ? 'default' : 'destructive'} className="font-normal">{product.totalQuantity > 0 ? 'In Stock' : 'Out of Stock'}</Badge></span>
                  </div>
                  {/* Expandable batch details for mobile */}
                  <div>
                    <Button
                      variant="ghost"
                      size="sm"
                      ...
                      onClick={() => toggleProductExpansion(product.productId)}
                    >
                      {expandedProducts.has(product.productId) ? 'Hide Batch Details' : 'Show Batch Details'}
                    </Button>
                    {expandedProducts.has(product.productId) && (
                      <div className="mt-2 bg-muted/30 p-2 rounded">
                        <h4 className="font-medium mb-2 text-sm">Batch Details</h4>
                        {product.batches && product.batches.length > 0 ? (
                          <div className="space-y-2">
                            {product.batches.map((batch) => (
                              <div key={batch.batchId} className="border rounded p-2 bg-background">
                                <div className="flex flex-wrap gap-2 text-xs">
                                  <span className="font-mono">Batch: {batch.batchId}</span>
                                  <span>Qty: {batch.quantity}</span>
                                  <span>Warehouse: {batch.warehouseName}</span>
                                  <span>Floor: {batch.floor || 'N/A'}</span>
                                  <span>Zone: {batch.zone || 'N/A'}</span>
                                  <span>Date: {new Date(batch.createdAt).toLocaleDateString()}</span>
                                </div>
                              </div>
                            ))}
                          </div>
                        ) : (
                          <div className="text-xs text-muted-foreground">No batches available for this product</div>
                        )}
                      </div>
                    )}
                  </div>
              ...
              <div className="flex items-center space-x-2">
                <div className="flex items-center space-x-1">
                  ...
                  <Button
                    ...
                    size="icon"
                    onClick={() => handlePageChange(page + 1)}
                    disabled={page * pageSize >= productBatchesData.totalCount}
                  >
