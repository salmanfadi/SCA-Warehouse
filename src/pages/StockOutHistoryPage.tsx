        ...
        const { data, error } = await executeQuery('stock-out-history', async (supabase) => {
          ...
          let query = supabase
            ...
            .select(`
              ...
              customer_name,
              destination,
              details:stock_out_details!stock_out_details_stock_out_id_fkey(
                id,
                product_id,
                quantity,
                processed_quantity,
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <p className="text-gray-600 mb-4">Filter stock-out history by various criteria</p>
                ...
                {data?.stock_outs?.length ? (
                  data.stock_outs.map((stockOut) => (
                    <TableRow key={stockOut.id}>
                      ...
                      <TableCell>
                        {stockOut.notes && (
                          <span className="text-sm text-gray-600">{stockOut.notes}</span>
                        )}
                      </TableCell>
                      <TableCell>
                            ...
                            onClick={async () => {
                              try {
                                ...
                                if (stockOut.status === 'completed') {
                                  processedItems = await fetchProcessedItems(stockOut.id);
                                }
